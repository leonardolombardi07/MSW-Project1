...
%   $Revision: 1.1.6.12 $  $Date: 2009/09/03 05:27:50 $
...
if ischar(numorder),
    ...
    if max(numorder) > nsections(Hd),
        error(generatemsgid('exceedsdims'), 'Index exceeds section length.');
    end
    
    if any(numorder < 1),
        error(generatemsgid('badsubscript'), ...
        'Indices must either be real positive integers or logicals.');
    end
    
    if length(numorder) ~= length(denorder) || length(svorder)-length(numorder) > 1,
        error(generatemsgid('arrayMismatch'), ...
        'The denominator and numerator indexes must be the same length.');
    end
