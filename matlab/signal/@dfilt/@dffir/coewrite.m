...
%   $Revision: 1.1.6.10 $  $Date: 2010/05/20 03:10:05 $ 
...
if ~strcmpi(Hq.Arithmetic, 'Fixed'),
    error(generatemsgid('NotFixedPoint'),...
        'XILINX .COE files can only be generated for fixed-point coefficients.');
end

if ~isreal(Hq),
    error(generatemsgid('NotReal'),...
        'XILINX .COE files can only be generated for filters with real coefficients.');
end
...
otherwise
    error(generatemsgid('InvalidParam'),'Invalid radix.');
end
