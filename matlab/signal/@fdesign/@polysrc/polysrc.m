...
%   <a href="matlab:web([matlabroot,'\toolbox\filterdesign\filtdesdemos\html\efficientsrcdemo.html'])">Efficient Sample Rate Conversion between Arbitrary Factors</a> demo. 
%

%   Copyright 2007-08 The MathWorks, Inc.
%   $Revision: 1.1.6.3 $  $Date: 2010/05/20 03:10:12 $
        ...
        if nargin > 2
            if strcmpi(lower(response),'Fractional Delay'),
                ...
                error(generatemsgid('InvalidResponse'), ...
                    'The ''%s'' enumerated value is invalid.',response);
            end
        end
    end
end
